...
- [ ] RC003 双击按标点删除真机验收：已安装焦点修复包的第二轮仍 failed（3 次单击实际删除但有 300ms 等待，5 次双击未删除）；进一步修复 WebView 范围越过自身 DocumentRange 和 Select 异步生效问题。自家 WebView 产品函数实际删除、保留末尾标点、重复及目标闲置后探针验证 passed，不能代替新包实体遥控器验收。首击提前删除与双击补偿已在自家 WebView 真实执行 passed（首删提交 28–48ms、标点补回、连续双击、取消恢复和删空）；最终安装版实体 RC003 与未经预热的闲置首击仍待验证。见 [Bug 记录](Bugs/2026-09-20-punctuation-webview-focus.md)。
...
- [ ] RC003 可选三键增强剩余验收：睡眠/崩溃恢复、多目标与共享宿主负对照、RC001、新返回键候选实体复验和第三方识别文字端到端；正常退出、清理退出状态与本轮升级/启动已 passed（见新安装证据），不代表崩溃清理通过；运行中安装未替换主程序的问题仍未归因，不得以安装器返回 0 代替实际文件核验。范围和边界见 [集成验收](Testing/WindowsRc003Input.md) 与 [ADR 0003](docs/decisions/0003-rc003-optional-input-helper.md)。
