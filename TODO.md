...
- [ ] 2026-09-21 最终日常预设安装与实体验收：TV 单击查看改动 Ctrl+Alt+B、双击开关侧边栏 Ctrl+B、长按撤销 Ctrl+Z；返回单击普通退格，Double/Long 均 disabled，按住仍连续退格。Home、菜单、方向、音量 Ctrl+PageUp/PageDown、OK、Esc 与语音保持现有方案。最终源码已更新，最新包尚未构建安装验收；首次/最近备份及显式应用保留。历史 `e8718f1` 返回双击撤销版已有预设 24 tests、安装后 36 格回读和设置保留证据，不能作为新 TV 配置通过。当前安装版只关闭返回双击后，用户确认快速连按与长按正常，见 [普通返回实体回验](Testing/evidence/plain-back-physical-20260921.json)；完整 Codex 前台效果、严格闲置首用和 RC001 仍 deferred。官方来源及聊天/标签页边界见 [方案调研](docs/investigations/2026-09-20-codex-remote-defaults.md)。
...
- [ ] RC003 三键增强显眼开关：移至遥控器图例上方，直接标明补齐返回、音量＋/－及管理员权限；开关反映增强启停，单独展示等待初始化、就绪和失败。组件 12、ButtonsPage 37、CodingPage 8 项定向测试 passed；保留用户显式开启、辅助程序提权、主程序普通权限与正常停止。本地升级及原生启停回验仍 deferred，完成后再标记完成。
- [ ] RC003 可选三键增强剩余验收：睡眠/崩溃恢复、多目标与共享宿主负对照、RC001、最终普通返回的严格闲置首用、新 TV/完整 Codex 预设和第三方识别文字端到端；旧规则已有本机实体首删/双击结果，关闭返回双击后快速连按已由用户确认，正常退出、清理退出状态与历史升级/启动已 passed，不代表新包或崩溃清理通过；运行中安装未替换主程序的问题仍未归因，不得以安装器返回 0 代替实际文件核验。范围和边界见 [集成验收](Testing/WindowsRc003Input.md) 与 [ADR 0003](docs/decisions/0003-rc003-optional-input-helper.md)。
