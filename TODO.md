...
- [ ] RC003 可选三键 HID lower filter：复用 QL-4 MIT 实现，仅将返回/音量±转为设备归因后的 F15/F13/F14；本地构建与自动验证已具备，签名/安装/回滚、闲置首用和语音回归尚 deferred。默认主程序不依赖驱动、不自动发布，边界见 Testing/WindowsRc003Filter.md。
