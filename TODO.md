...
- [ ] 即时普通退格＋双击 Ctrl+Z：单击立即普通退格，按住继续重复；构成双击时首击已删除，再发送 Ctrl+Z。撤销范围与分组由编辑器决定，不保证整段撤销或精确只还原首删。引擎/手势接线测试 21/32 passed；2026-09-21 自家 WebView 普通输入实测 12→11→12 passed（只读 UIA 验证，不走文本事务）。来源 e8718f1 的本地 NSIS 构建、正常升级、启动和配置回读 passed；RC003/RC001 实体按键及未经预热闲置首用仍 deferred。
...
- [ ] 2026-09-20 新日常预设安装与实体验收：音量＋/－为 Ctrl+PageUp/PageDown，TV 为侧边栏/改动/终端，菜单为命令/模型/待处理项，返回双击为 Ctrl+Z 撤销；保留首次/最近备份并由用户显式应用。撤销版预设定向 24 tests passed，与引擎实际输入分开记录，本地新包已安装、启动并应用预设：36 格读回一致，当前配置备份成功、已有应用列表和语音/音频设置不变，原生返回说明正确；Codex 前台实际效果、冷首用和双型号实体行为仍 deferred。快捷键来源与“聊天或标签页”边界见 [新方案调研](docs/investigations/2026-09-20-codex-remote-defaults.md)。
...
- [ ] RC003 可选三键增强剩余验收：睡眠/崩溃恢复、多目标与共享宿主负对照、RC001、即时退格＋双击撤销及严格闲置首用、新 Codex 预设和第三方识别文字端到端；旧规则已有本机实体首删/双击结果，正常退出、清理退出状态与本轮升级/启动已 passed，不代表崩溃清理通过；运行中安装未替换主程序的问题仍未归因，不得以安装器返回 0 代替实际文件核验。范围和边界见 [集成验收](Testing/WindowsRc003Input.md) 与 [ADR 0003](docs/decisions/0003-rc003-optional-input-helper.md)。
