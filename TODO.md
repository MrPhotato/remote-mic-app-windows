...
- [ ] RC003 双击按标点删除真机验收：已安装焦点修复包的第二轮仍 failed（3 次单击实际删除但有 300ms 等待，5 次双击未删除）；进一步修复 WebView 范围越过自身 DocumentRange 和 Select 异步生效问题。自家 WebView 产品函数实际删除、保留末尾标点、重复及目标闲置后探针验证 passed，不能代替新包实体遥控器验收。首击提前删除与双击补偿已进入候选实现，实际速度、补偿和闲置首击仍待验证。见 [Bug 记录](Bugs/2026-09-20-punctuation-webview-focus.md)。
