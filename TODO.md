...
- [ ] 即时普通退格＋双击 Ctrl+Z：单击立即普通退格，按住继续重复；构成双击时首击已删除，再发送 Ctrl+Z。撤销范围与分组由编辑器决定，不保证整段撤销或精确只还原首删。引擎/手势接线测试 21/32 passed；2026-09-21 自家 WebView 普通输入实测 12→11→12 passed（只读 UIA 验证，不走文本事务）。新安装、RC003/RC001 实体按键及未经预热闲置首用仍 deferred。
...
- [ ] 2026-09-20 新日常预设安装与实体验收：音量＋/－为 Ctrl+PageUp/PageDown，TV 为侧边栏/改动/终端，菜单为命令/模型/待处理项，返回双击为 Ctrl+Z 撤销；保留首次/最近备份并由用户显式应用。撤销版预设定向 24 tests passed，与引擎实际输入分开记录，当前尚未构建安装或验证 Codex 前台实际效果、冷首用和双型号实体行为。快捷键来源与“聊天或标签页”边界见 [新方案调研](docs/investigations/2026-09-20-codex-remote-defaults.md)。
