...
mod tests {
    ...
    fn voice_hold_hotkey_round_trips_and_validates_chord() {
        ...
        let codex = KeyChord {
            keys: vec![
                sayall_windows::send_input::KeyCode::LeftControl,
                sayall_windows::send_input::KeyCode::LeftShift,
                sayall_windows::send_input::KeyCode::D,
            ],
        };
        assert_eq!(
            store.save_voice_hold_hotkey(Some(codex.clone())).unwrap(),
            Some(codex.clone())
        );
        assert_eq!(store.load_voice_hold_hotkey().unwrap(), Some(codex));
