...
export type ButtonAction =
  | { type: "disabled" }
  | { type: "normal_backspace" }
  | { type: "delete_to_punctuation" }
 ...
 *
 * - **all**（直接归因族：电源 VK 0xFF/0x5F、菜单 VK_APPS、返回厂商键）：
 *   可配置任意动作；返回键是否实际上报取决于硬件，RC003 需单独实测；
...
export function shortcutCapability(
  ...
  if (button === "power" || button === "menu" || button === "back") {
    return "all";
  }
  if (
    button === "volume_up" ||
...
export function actionSummary(action: ButtonAction | undefined): string {
  ...
  if (action.type === "normal_backspace") return "退格（按住连续删除）";
  if (action.type === "delete_to_punctuation") return "删到上一个标点（保留标点）";
